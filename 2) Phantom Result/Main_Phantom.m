...
%% Amplitude
temp = 0; % =0 represents no source and =1,2,3,4 represents that much number of dipoles
n_sources = 1; %Number of sources can be varied from 1 to 10 or more
range = 155:195;
nmax = n_sources+1;
Repetitions = 100;
T_Samples = 350;
...
for i=1:Repetitions
    Noise_Data{i,1} = Multfactor*Noise{i,1}; %Y{i,1}(:,10+1:10+size(Y{i,1}(:,range),2));
    if temp == 0
        Signal_Data{i,1} = Multfactor*Y{i,1}(:,50+1:50+size(Y{i,1}(:,range),2));
    else
        Signal_Data{i,1} =  Multfactor*Y{i,1}(:,range); % Signal
    end
    Est_SNR(i,1) = snr(Signal_Data{i,1},Noise_Data{i,1}); % SNR
end
Est_SNR = round(Est_SNR,1);
[max(Est_SNR) min(Est_SNR)]

for i = 1:length(Y)
    PWMEG_MAT{i,1} =  Noise_Data{i,1};
    sspw{i,1} = sum(PWMEG_MAT{i,1}.^2);
end

temp12 = PWMEG_MAT{1,1};
pd = makedist('Normal');
figure; qqplot(temp12(:),pd); grid on; axis on;

%% F-Ratio Statistics
parfor i=1:size(Source_Ind,2)
    i
    [Fratiosupek_Combined{i,1},S_Reduced{i,1},SSE_Reduced{i,1},SSE_Full{i,1}] = Fstatistics_Free_OrientSimrand4(Signal_Data{i,1},Gain,AP_max_iters,mode,GridLoc,nmax,Multfactor);
end

% if temp == 0
% save LPC_VOLsour0.mat
% else
% save (['LPC_VOLsour' num2str(n_sources) '.mat'])
% end
