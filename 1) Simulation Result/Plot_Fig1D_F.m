%% Thrteshold values are dependent upon the SNR
clear
close all
clc
...
for i=1:MaxnumSources+1
    ...
    for j=1:length(SNRs)
        ...
        xx = THRESH;
        for ax=1:length(THRESH)
            yy = [yy Accuracyfull{j,idxCorr}{i,ax}(1,3)];
        end
        zz = (i-1)*ones(1,length(THRESH));
...
%     ylabel('Number of Sources Estimation Accuracy (%)','FontSize', 15)
    ...
    legend('SNR=-8dB','SNR=-4dB','SNR=0dB','SNR=4dB','SNR=8dB','fontweight','bold','FontSize', 15)
  %  title(['Subj ' num2str(Anat) ', NOS=', num2str(i-1) ', Corr=' num2str(Corr) ', and MError=', num2str(ModelingError) ],'FontSize', 10)
    hold off
    view([0 90])
    x0=100;
    y0=10;
