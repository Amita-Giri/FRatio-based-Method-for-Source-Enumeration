...
%% Thrteshold values are independent upon the Correlation
idxCorr = 0;
Anat = 1;
ModelingError = 'X';
...
for j=1:length(Accuracyfull{1,1}(:,1))
    ...
    legend('\rho=0.1','\rho=0.5','\rho=0.9','FontSize', 15)
    title(['Subj ' num2str(Anatomy) ', SNR=', num2str(SNR) ', NOS=', num2str(j-1) ', and MError=', num2str(ModelingError) ],'FontSize', 10)
    hold off
    view([0 90])
    x0=100;
    y0=10;
